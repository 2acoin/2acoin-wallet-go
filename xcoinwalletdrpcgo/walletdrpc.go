...
// Package xcoinwalletdrpcgo handles the the rpc connection between your app and 2acoin-service
package xcoinwalletdrpcgo
...
var (
	rpcURL = "http://127.0.0.1:17760/json_rpc"
)

// RequestBalance provides the available and locked balances of the current wallet
// returned balances are expressed in ARMS, not in 0.01 ARMS
...
// parameters amount and fee are expressed in ARMS, not 0.01 ARMS
func SendTransaction(addressRecipient string, amount float64, paymentID string, fee float64, rpcPassword string) (transactionHash string, err error) {
	...
	amountInt := uint64(amount * 100000000) // expressed in hundredth of ARMS
	feeInt := uint64(fee * 100000000)       // expressed in hundredth of ARMS
...
// threshold is the value that determines which outputs will be optimized. Only the outputs, lesser than the threshold value, will be included into a fusion transaction (threshold is expressed in ARMS, not 0.01 ARMS).
...
func EstimateFusion(threshold int, addresses []string, rpcPassword string) (fusionReadyCount int, totalOutputCount int, err error) {
	...
	threshold *= 100000000 // expressed in hundredth of ARMS
...
// returned fee is expressed in ARMS, not in 0.01 ARMS
